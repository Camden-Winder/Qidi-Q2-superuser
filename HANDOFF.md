...
## Max4-RC1.01 — What's In It (historical; Max 4 removed from repo, see the docs-only entry at the top)

- `AIO_VERSION='Max4-RC1.01'` — a Max 4-only version namespace that no longer exists. The single live scheme is `RC<major>.<minor>` (see `CLAUDE.md`).
- No functional changes to the Max 4 installer.
- **Max4/Instructions.md** — AIO menu preview section added showing current Max 4 menu layout. `Max4/` was deleted in full; this file is gone.

---

## RC2.14 — KAMP files moved into `${CONFIG_DIR}/KAMP/`
...
> Historical note: the `[include KAMP/KAMP_Settings.cfg]` capitalisation above is what RC2.14 actually shipped. It was the bug fixed in RC2.48 — the correct filename is `KAMP_settings.cfg` with a lowercase `s`. See LESSONS `[L007]`.
>
> This entry was reconstructed during the docs audit from a fossilised `## Project` / `## Current State` /
> `## Established Conventions` / `## Key Files` block that sat between RC2.37 and RC2.36. Those four sections
> were deleted: all were superseded by `CLAUDE.md`, and the Key Files table still pointed at pre-RC2.14 /
> pre-RC2.46 paths plus a `Configurations/` + `Plugins/` pair that no longer exists. A `HAPPIER_HARE_RELEASE_ZIP`
> manual-release TODO went with them — that constant is no longer in `aio_menu.sh` (HelixScreen now installs
> from `HELIXSCREEN_INSTALLER`), so the action item was moot.
