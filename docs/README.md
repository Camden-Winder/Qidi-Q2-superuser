...
|---------|------------------|
| RC3.02 | Fixed "Disable unnecessary processes" blacking out the stock touchscreen on both Just Faster installs; fixed Update Macros writing to the wrong directory (it reported success while leaving your macros untouched); fixed a hang when installing via `curl \| bash`; restored the "don't delete files that were here before AIO" protection, which had never actually been active; made Testing options 4–9 reachable; blocked Revert to Backup on 01.01.02+ firmware, where it was unsafe — current release |
